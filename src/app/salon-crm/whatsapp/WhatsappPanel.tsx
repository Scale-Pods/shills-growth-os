...
export default function WhatsappPanel({ salons }: { salons: Salon[] }) {
  ...
  const conversations = useMemo(() => {
    return salons
      .map((s) => ({ salon: s, entries: normalizeTranscript(s.conversation_transcript) }))
  ...
  const allEntries = useMemo(() => salons.flatMap((s) => normalizeTranscript(s.conversation_transcript)), [salons]);
